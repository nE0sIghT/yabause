...
int Cs2SaveState(StateStream *fp) {
   int offset, i, i2;
   IOCheck_struct check = { 0, 0 };

   // This is mostly kludge, but it will have to do until I have time to rewrite it all

   offset = StateStreamWriteHeader(fp, "CS2 ", 3);

   // Write cart type
   StateWriteChecked(&check, (void *) &Cs2Area->carttype, 4, 1, fp);

   // Write cd block registers
   StateWriteChecked(&check, (void *) &Cs2Area->reg, sizeof(blockregs_struct), 1, fp);

   // Write current Status variables(needs a rewrite)
   StateWriteChecked(&check, (void *) &Cs2Area->FAD, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->status, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->options, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->repcnt, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->ctrladdr, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->track, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->index, 1, 1, fp);

   // Write other cd block internal variables
   StateWriteChecked(&check, (void *) &Cs2Area->satauth, 2, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->mpgauth, 2, 1, fp);

   StateWriteChecked(&check, (void *) &Cs2Area->transfercount, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->cdwnum, 4, 1, fp);
   StateWriteChecked(&check, (void *) Cs2Area->TOC, 4, 102, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->playFAD, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->playendFAD, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->maxrepeat, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->getsectsize, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->putsectsize, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->calcsize, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->infotranstype, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->datatranstype, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->isonesectorstored, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->isdiskchanged, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->isbufferfull, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->speed1x, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->isaudio, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->transfileinfo, 1, 12, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->lastbuffer, 1, 1, fp);
   StateWriteChecked(&check, (void *)&Cs2Area->transscodeq, 5*2, 1, fp);
   StateWriteChecked(&check, (void *)&Cs2Area->transscoderw, 12*2, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->_command, 1, 1, fp);
   {
      u32 temp = (Cs2Area->_periodictiming + 3) / 3;
      StateWriteChecked(&check, (void *) &temp, 4, 1, fp);
   }
   StateWriteChecked(&check, (void *) &Cs2Area->_commandtiming, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->outconcddevnum, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->outconmpegfbnum, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->outconmpegbufnum, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->outconmpegromnum, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->outconhostnum, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->datatranspartitionnum, 1, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->datatransoffset, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->datanumsecttrans, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->datatranssectpos, 2, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->datasectstotrans, 2, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->blockfreespace, 4, 1, fp);
   StateWriteChecked(&check, (void *) &Cs2Area->curdirsect, 4, 1, fp);

   // Write CD buffer
   StateWriteChecked(&check, (void *)Cs2Area->block, sizeof(block_struct), MAX_BLOCKS, fp);

   // Write partition data
   for (i = 0; i < MAX_SELECTORS; i++)
   {
      StateWriteChecked(&check, (void *)&Cs2Area->partition[i].size, 4, 1, fp);
      StateWriteChecked(&check, (void *)Cs2Area->partition[i].blocknum, 1, MAX_BLOCKS, fp);
      StateWriteChecked(&check, (void *)&Cs2Area->partition[i].numblocks, 1, 1, fp);
      ...
      for (i2 = 0; i2 < MAX_BLOCKS; i2++)
      {
        ...
        StateWriteChecked(&check, &index, 4, 1, fp);
      }
   }

   // Write filter data
   StateWriteChecked(&check, (void *)Cs2Area->filter, sizeof(filter_struct), MAX_SELECTORS, fp);

   // Write File Info Table
   StateWriteChecked(&check, (void *)Cs2Area->fileinfo, sizeof(dirrec_struct), MAX_FILES, fp);

   // Write MPEG card registers here

   // Write current MPEG card status variables
   StateWriteChecked(&check, (void *)&Cs2Area->actionstatus, 1, 1, fp);
   StateWriteChecked(&check, (void *)&Cs2Area->pictureinfo, 1, 1, fp);
   StateWriteChecked(&check, (void *)&Cs2Area->mpegaudiostatus, 1, 1, fp);
   StateWriteChecked(&check, (void *)&Cs2Area->mpegvideostatus, 2, 1, fp);
   StateWriteChecked(&check, (void *)&Cs2Area->vcounter, 2, 1, fp);

   // Write other MPEG card internal variables
   StateWriteChecked(&check, (void *)&Cs2Area->mpegintmask, 4, 1, fp);
   StateWriteChecked(&check, (void *)Cs2Area->mpegcon, sizeof(mpegcon_struct), 2, fp);
   StateWriteChecked(&check, (void *)Cs2Area->mpegstm, sizeof(mpegstm_struct), 2, fp);

   StateWriteChecked(&check, (void *)&Cs2Area->playtype, 4, 1, fp);

   StateWriteChecked(&check, (void *)&Cs2Area->_command_execlock, 4, 1, fp);
...
int Cs2LoadState(StateStream *fp, int version, int size) {
   int i, i2;
   IOCheck_struct check = { 0, 0 };
   ...
   // Read cart type
   StateReadChecked(&check, (void *)&Cs2Area->carttype, 4, 1, fp);

   // Read cd block registers
   StateReadChecked(&check, (void *)&Cs2Area->reg, sizeof(blockregs_struct), 1, fp);

   // Read current Status variables(needs a reRead)
   StateReadChecked(&check, (void *)&Cs2Area->FAD, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->status, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->options, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->repcnt, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->ctrladdr, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->track, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->index, 1, 1, fp);

   // Read other cd block internal variables
   StateReadChecked(&check, (void *)&Cs2Area->satauth, 2, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->mpgauth, 2, 1, fp);

   StateReadChecked(&check, (void *)&Cs2Area->transfercount, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->cdwnum, 4, 1, fp);
   StateReadChecked(&check, (void *)Cs2Area->TOC, 4, 102, fp);
   StateReadChecked(&check, (void *)&Cs2Area->playFAD, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->playendFAD, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->maxrepeat, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->getsectsize, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->putsectsize, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->calcsize, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->infotranstype, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->datatranstype, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->isonesectorstored, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->isdiskchanged, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->isbufferfull, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->speed1x, 1, 1, fp);
   if (version > 1)
      StateReadChecked(&check, (void *)&Cs2Area->isaudio, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->transfileinfo, 1, 12, fp);
   StateReadChecked(&check, (void *)&Cs2Area->lastbuffer, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->transscodeq, 5 * 2, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->transscoderw, 12 * 2, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->_command, 1, 1, fp);
   {
      u32 temp;
   ...
   StateReadChecked(&check, (void *)&Cs2Area->_commandtiming, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->outconcddevnum, 1, 1, fp);
   if (Cs2Area->outconcddevnum == 0xFF)
      Cs2Area->outconcddev = NULL;
   else
      Cs2Area->outconcddev = Cs2Area->filter + Cs2Area->outconcddevnum;

   StateReadChecked(&check, (void *)&Cs2Area->outconmpegfbnum, 1, 1, fp);
   if (Cs2Area->outconmpegfbnum == 0xFF)
      Cs2Area->outconmpegfb = NULL;
   else
      Cs2Area->outconmpegfb = Cs2Area->filter + Cs2Area->outconmpegfbnum;

   StateReadChecked(&check, (void *)&Cs2Area->outconmpegbufnum, 1, 1, fp);
   if (Cs2Area->outconmpegbufnum == 0xFF)
      Cs2Area->outconmpegbuf = NULL;
   else
      Cs2Area->outconmpegbuf = Cs2Area->filter + Cs2Area->outconmpegbufnum;

   StateReadChecked(&check, (void *)&Cs2Area->outconmpegromnum, 1, 1, fp);
   if (Cs2Area->outconmpegromnum == 0xFF)
      Cs2Area->outconmpegrom = NULL;
   else
      Cs2Area->outconmpegrom = Cs2Area->filter + Cs2Area->outconmpegromnum;

   StateReadChecked(&check, (void *)&Cs2Area->outconhostnum, 1, 1, fp);
   if (Cs2Area->outconhostnum == 0xFF)
      Cs2Area->outconhost = NULL;
   else
      Cs2Area->outconhost = Cs2Area->filter + Cs2Area->outconhostnum;

   StateReadChecked(&check, (void *)&Cs2Area->datatranspartitionnum, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->datatransoffset, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->datanumsecttrans, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->datatranssectpos, 2, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->datasectstotrans, 2, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->blockfreespace, 4, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->curdirsect, 4, 1, fp);

   // Read CD buffer
   StateReadChecked(&check, (void *)Cs2Area->block, sizeof(block_struct), MAX_BLOCKS, fp);

   // Read partition data
   for (i = 0; i < MAX_SELECTORS; i++)
   {
      StateReadChecked(&check, (void *)&Cs2Area->partition[i].size, 4, 1, fp);
      StateReadChecked(&check, (void *)Cs2Area->partition[i].blocknum, 1, MAX_BLOCKS, fp);
      StateReadChecked(&check, (void *)&Cs2Area->partition[i].numblocks, 1, 1, fp);

      u32 index=0;
      for (i2 = 0; i2 < MAX_BLOCKS; i2++)
      {
        StateReadChecked(&check, (void *)&index, 4, 1, fp);
        if (index == 0xFFFFFFFF){
          Cs2Area->partition[i].block[i2] = NULL;
        }
        else{
   ...
   // Read filter data
   StateReadChecked(&check, (void *)Cs2Area->filter, sizeof(filter_struct), MAX_SELECTORS, fp);

   // Read File Info Table
   StateReadChecked(&check, (void *)Cs2Area->fileinfo, sizeof(dirrec_struct), MAX_FILES, fp);

   // Read MPEG card registers here

   // Read current MPEG card status variables
   StateReadChecked(&check, (void *)&Cs2Area->actionstatus, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->pictureinfo, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->mpegaudiostatus, 1, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->mpegvideostatus, 2, 1, fp);
   StateReadChecked(&check, (void *)&Cs2Area->vcounter, 2, 1, fp);

   // Read other MPEG card internal variables
   StateReadChecked(&check, (void *)&Cs2Area->mpegintmask, 4, 1, fp);
   StateReadChecked(&check, (void *)Cs2Area->mpegcon, sizeof(mpegcon_struct), 2, fp);
   StateReadChecked(&check, (void *)Cs2Area->mpegstm, sizeof(mpegstm_struct), 2, fp);

   StateReadChecked(&check, (void *)&Cs2Area->playtype, 4, 1, fp);

   if (version > 2) StateReadChecked(&check, (void *)&Cs2Area->_command_execlock, 4, 1, fp);
