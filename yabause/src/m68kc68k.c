...
static void C68k_Save_State(c68k_struc *mcpu, StateStream *fp)
{
   ...
   for (i = 0; i < 8; i++)
      StateWriteChecked(&check, (void *)&mcpu->D[i], sizeof(u32), 1, fp);

   for (i = 0; i < 8; i++)
      StateWriteChecked(&check, (void *)&mcpu->A[i], sizeof(u32), 1, fp);

   StateWriteChecked(&check, (void *)&mcpu->flag_C, sizeof(u32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->flag_V, sizeof(u32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->flag_notZ, sizeof(u32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->flag_N, sizeof(u32), 1, fp);

   StateWriteChecked(&check, (void *)&mcpu->flag_X, sizeof(u32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->flag_I, sizeof(u32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->flag_S, sizeof(u32), 1, fp);

   StateWriteChecked(&check, (void *)&mcpu->USP, sizeof(u32), 1, fp);

   pc = C68k_Get_PC(&C68K);

   StateWriteChecked(&check, (void *)&pc, sizeof(u32), 1, fp);

   StateWriteChecked(&check, (void *)&mcpu->Status, sizeof(u32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->IRQLine, sizeof(s32), 1, fp);

   StateWriteChecked(&check, (void *)&mcpu->CycleToDo, sizeof(s32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->CycleIO, sizeof(s32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->CycleSup, sizeof(s32), 1, fp);
   StateWriteChecked(&check, (void *)&mcpu->dirty1, sizeof(u32), 1, fp);
}

static void M68KC68KSaveState(StateStream *fp) {
   C68k_Save_State(&C68K, fp);
}

static void C68k_Load_State(c68k_struc *mcpu, StateStream *fp)
{
   IOCheck_struct check = { 0, 0 };
   int i = 0;
   u32 pc = 0;
   ...
   for (i = 0; i < 8; i++)
      StateReadChecked(&check, (void *)&mcpu->D[i], sizeof(u32), 1, fp);

   for (i = 0; i < 8; i++)
      StateReadChecked(&check, (void *)&mcpu->A[i], sizeof(u32), 1, fp);

   StateReadChecked(&check, (void *)&mcpu->flag_C, sizeof(u32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->flag_V, sizeof(u32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->flag_notZ, sizeof(u32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->flag_N, sizeof(u32), 1, fp);

   StateReadChecked(&check, (void *)&mcpu->flag_X, sizeof(u32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->flag_I, sizeof(u32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->flag_S, sizeof(u32), 1, fp);

   StateReadChecked(&check, (void *)&mcpu->USP, sizeof(u32), 1, fp);

   StateReadChecked(&check, (void *)&pc, sizeof(u32), 1, fp);

   C68k_Set_PC(&C68K, pc);

   StateReadChecked(&check, (void *)&mcpu->Status, sizeof(u32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->IRQLine, sizeof(s32), 1, fp);

   StateReadChecked(&check, (void *)&mcpu->CycleToDo, sizeof(s32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->CycleIO, sizeof(s32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->CycleSup, sizeof(s32), 1, fp);
   StateReadChecked(&check, (void *)&mcpu->dirty1, sizeof(u32), 1, fp);
}

static void M68KC68KLoadState(StateStream *fp) {
