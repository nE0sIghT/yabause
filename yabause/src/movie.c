...
void SaveMovieInState(StateStream *stream) {

	struct MovieBufferStruct tempbuffer;

	StateStreamSeek(stream, 0, SEEK_END);

	if(Movie.Status == Recording || Movie.Status == Playback) {
		tempbuffer=ReadMovieIntoABuffer(Movie.fp);

		StateStreamWrite(&tempbuffer.size, 4, 1, stream);
		StateStreamWrite(tempbuffer.data, tempbuffer.size, 1, stream);
	}
}

//////////////////////////////////////////////////////////////////////////////

void MovieReadState(StateStream *stream) {
...
void ReadMovieInState(StateStream *stream) {
   ...
   size_t num_read = 0;
	...
	if(Movie.Status == Recording || (Movie.Status == Playback && Movie.ReadOnly == 0)) {
		...
		fpos=(int)StateStreamTell(stream);//where we are in the savestate
      ...
      num_read = StateStreamRead(&tempbuffer.size, 4, 1, stream);//size
		if ((tempbuffer.data = (char *)malloc(tempbuffer.size)) == NULL)
		{
			return;
		}
      num_read = StateStreamRead(tempbuffer.data, 1, tempbuffer.size, stream);//movie
		StateStreamSeek(stream, fpos, SEEK_SET);//reset savestate position
